...
export default function GalleryAdminS3() {
  ...
  const loadFromS3 = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/save-gallery-s3");
      if (res.ok) {
  ...
  const handleSaveToS3 = async () => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Gallery saved to S3 successfully!");
      } else {
